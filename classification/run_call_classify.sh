...
do
    ...
    cmd="qsub -v dataset_ID=$dataset_ID,conda_env=$conda_env -q $queue -l select=1:ncpus${ncpus}:mem=${mem}GB:mpiprocs=${mpi_procs} -l walltime=${walltime_hrs}:00:00 -M your_email@email.com -N ${dataset_ID}_classify -o /cluster/output/path/classify_${dataset_ID}_out.txt call_classify.pbs"
    echo $cmd
    $cmd
done